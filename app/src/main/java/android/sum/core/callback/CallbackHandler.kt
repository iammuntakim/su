...
import android.sum.locator.ServiceLocator
import android.sum.extensions.getLabel
import android.sum.extensions.getPackageInfo
import android.sum.extensions.toast
import android.sum.model.policy.SuAccessPolicy
import android.sum.model.policy.createLogEntry
...
object CallbackHandler {
    ...
    private fun handleLogging(context: Context, data: Bundle) {
        ...
        val policy = data.getIntComp("policy", Policy.ALLOW)
        ...
        val log = runCatching {
            pm.getPackageInfo(fromUid, pid)?.applicationInfo?.let {
                pm.createLogEntry(it, toUid, pid, command, policy, target, seContext, gids)
            }
        }.getOrNull() ?: createLogEntry(fromUid, toUid, pid, command, policy, target, seContext, gids)

        if (notify)
            notify(context, log.action >= Policy.ALLOW, log.appName)
    ...
    private fun handleNotify(context: Context, data: Bundle) {
        ...
        notify(context, policy >= Policy.ALLOW, appName)
    }
