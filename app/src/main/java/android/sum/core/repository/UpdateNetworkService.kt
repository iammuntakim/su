...
import android.sum.database.GithubApiServices
import android.sum.database.RawUrl
import android.sum.extensions.dateFormat
...
class NetworkService(
    ...
    private inline fun <T> safe(factory: () -> T): T? {
        return try {
            if (DeviceInfo.isConnected.value == true)
                factory()
            else
                null
        } catch (e: Exception) {
