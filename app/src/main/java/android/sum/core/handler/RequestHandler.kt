...
import android.sum.data.policy.PolicyDao
...
class RequestHandler(
    ...
    suspend fun start(intent: Intent): Boolean {
        ...
        // Never allow android.sum (could be malware)
        ...
        when (Config.suAutoResponse) {
            Config.Value.SU_AUTO_DENY -> {
                respond(Policy.DENY, 0)
                return false
            }
            Config.Value.SU_AUTO_ALLOW -> {
                respond(Policy.ALLOW, 0)
                return false
            }
        }
    ...
    private suspend fun init(intent: Intent): Boolean {
        ...
        output = File(fifo)
        policy = policyDB.fetch(uid) ?: Policy(uid)
        try {
    ...
    suspend fun respond(action: Int, time: Long) {
        if (action == Policy.ALLOW && Config.suRestrict) {
            policy.policy = Policy.RESTRICT
        } else {
