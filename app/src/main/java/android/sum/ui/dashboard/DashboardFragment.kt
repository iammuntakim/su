package android.sum.ui.dashboard
...
import android.sum.Info
import android.sum.core.Const
import android.sum.ui.drawable.DrawableFactory
import android.sum.ui.widget.MaterialWidgets
import android.sum.ui.widget.MaterialWidgets.dp
...
class DashboardFragment : Fragment() {
    ...
    private fun buildLayout(context: android.content.Context): ScrollView {
        ...
        val rootStatus = Info.env.isActive
        val rootVersion = if (rootStatus) "v${Info.env.version}" else "Not installed"
        ...
        val zygiskStatus = if (Info.env.isZygisk) "Enabled" else "Disabled"
        val zygiskColor = if (Info.env.isZygisk) 0xFF1E8E3E.toInt() else 0xFF5F6368.toInt()
        ...
        val connectionLabel = if (Info.isConnected.value == true) "Connected" else "Disconnected"
        val connectionColor = if (Info.isConnected.value == true) 0xFF1E8E3E.toInt() else 0xFFD93025.toInt()
